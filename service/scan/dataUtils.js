...
dataUtils.dataParse = async function (contract, contractAddr, blockNumber, txHash, contractName, collection_id, owner, chain_symbol, type, fromBlock,
    toBlock) {
    const scanResult = await contract.queryFilter(eventFilter, fromBlock, toBlock);
    await Promise.all(scanResult.map(async (value) => {
        try {
            ...
            console.log({ contractAddr, from, to, tokenId });
            await plaNFTDB.erc721Transfer.addNewData([contractAddr, from, to, tokenId, chain_symbol, txHash]);
        } catch (error) {
