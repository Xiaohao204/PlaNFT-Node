...
nftInfo.insertNFTInfo = function (connection, params) {
    return new Promise(function (resolve, reject) {
        const sql = 'INSERT into nft_info (sales_id,end_block_id,collection_id,token_id,contract_address,user_address,description,properties,image_url,title,is_frozen,token_uri,metadata,chain_symbol) Values (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE end_block_id=VALUES(end_block_id)';
